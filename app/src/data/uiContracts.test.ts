...
test('market actions expose concise player-specific button labels', () => {
  ...
  assert.match(marketSource, /accessibilityLabel=\{`Last \$\{option\.slice\(1\)\} games`\}/);
});

test('detail dividend total follows the selected last-game range', () => {
  assert.match(marketSource, /const rangeTotal = visiblePoints\.reduce/);
  assert.match(marketSource, /\{formatSignedMoney\(rangeTotal\)\} Last \{visiblePoints\.length\} games/);
  assert.doesNotMatch(marketSource, /this season/i);
});
