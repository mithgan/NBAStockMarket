...
import { rankGameLeaderboard } from '../state/game';
import { colors } from '../theme';

const prototypeRivals = leaderboardRivals.map((rival, index) => ({
  name: rival.name,
  value: rival.value,
  dailyChange: [72_000, -41_000, 28_000, 9_000][index] ?? 0,
}));

export function LeaderboardScreen() {
  const { state, summary } = usePortfolio();
  const leaderboard = rankGameLeaderboard(
    prototypeRivals,
    summary.totalValue,
    state.settledDates.length,
  );
  const winner = leaderboard[0];

  return (
    <ScrollView style={styles.scroll} contentContainerStyle={styles.content}>
      <Text style={styles.eyebrow}>PROTOTYPE LEAGUE</Text>
      <Text accessibilityRole="header" style={styles.title}>Portfolio leaderboard</Text>
      <Text style={styles.subtle}>Your row is live. Rival portfolios use deterministic replay trajectories until accounts and multiplayer are connected.</Text>
      ...
      <View style={styles.table}>
        {leaderboard.slice(1).map((entry) => (
          <View key={entry.name} style={[styles.row, entry.isUser && styles.youRow]}>
